...
formatSearchString <- function(userSearchString) {
  return (gsub(" ", "+", userSearchString))
}

# Returns the longitude and latitude of the given city that twitteR has trending
# data on
findLongLat <- function(chosen.city) {
  geo.data.colname <- paste0("geo.location.data$", chosen.city)
  return (eval(parse(text = geo.data.colname)))
}

# Finds the geocode to use in the searchTwitter(...) function based on the 
# chosen.city
getGeocode <- function(chosen.city) {
  coordinates <- findLongLat(chosen.city)
  result <- paste0(coordinates, ",50mi")
  return (result)
}

# This function gets the recent posts in the chosen city that contain a given
# search string. If there are fewer posts than asked for, this function will
# return as many posts as there are. If there are 0 recent posts, a message
# saying that there weren't any posts will be returned.
getRecentPostsInChosenLocation <- function(str, num.posts, chosen.city) {
  if (num.posts == 'error') {
...
tweetdataframe <- do.call('rbind', lapply(getRecentPostsInChosenLocation("#lksdjhflakjsdhf", 15, "Atlanta"), as.data.frame))
